...
| 이름 | 회고 |
|------|------|
| **명인** | 프로그램 시작 후 첫번째 프로젝트여서 갈피도 잘 못잡고 수업과 달리 실제 데이터를 다루는 게 서툴렀지만 다행히 팀원들이 모두 열심히 프로젝트를 참여해서 많은 부분에서 도움을 받아 잘 마무리할 수 있었던거 같다. |
| **수빈** | 첫 프로젝트를 시작하며 잘 할 수 있을까 걱정을 한가득 안고 시작했는데 팀원들이 잘 이끌어줘서 무사히 잘 끝낼 수 있었던 것 같아요~ 결과물 보면 넘 뿌듯!! ^오^ 함께 준비하며 덕분에 많이 배우고 한단계 더 성장하였습니다. 모두들 Thank you~ |
| **형우** | 기계공학과에서 삭막한 프로젝트만 해오다가 산뜻한 프로젝트 할 수 있어서 좋았습니다. 파이썬과 DB의 기초를 되돌아볼 수 있었어요. 🤗 |
| **경재** | 이번 프로젝트에서는 서울시 주유소 정보를 웹에서 직접 수집하고, 이를 시각화하여 사용자에게 제공하는 대시보드를 만드는 과정을 경험했다. 웹사이트 구조가 동적으로 구성되어 있어 Selenium을 사용해 데이터를 수집했는데, 예상보다 크롤링이 까다로웠다. 특히 비동기적으로 로딩되는 정보들 때문에 요소 탐색과 대기 로직을 반복적으로 조정해야 했다. 이번 경험은 단순한 기술 습득을 넘어, 데이터를 수집하고 분석해 실제 사용자에게 전달하는 과정을 처음부터 끝까지 체험할 수 있는 좋은 기회였다. 특히 Streamlit의 고급 기능들을 익히고, 실제 서비스를 염두에 둔 개발을 해봤다는 점에서 유익했다. |
| **동욱** | 프로젝트가 어떤 과정으로 진행되는지 전반적으로 파악할 수 있어서 좋았습니다. 1차 프로젝트인 만큼 스스로에게 아쉬운 부분도 있었지만, 팀원 분들과 함께 멋진 결과물을 도출해낸 것 같아 만족스럽습니다. |
